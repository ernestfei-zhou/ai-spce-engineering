...
> **性质：** 产品功能设计计划，不是实现任务清单。表模型与具体技术架构暂不展开。  
> **AI 基座（强制）：** AgentScope **Java** 2.0 为唯一运行时底座。不以 Python 版 AgentScope 为内核，不做 Python/Java 双栈推理。
...
| 运行时基座 | **仅 AgentScope Java 2.0**（`ReActAgent` + `HarnessAgent`）。**不使用** AgentScope Python 2.0 作为内核、Agent Service 或双栈推理 |
...
研发人员登录后先看到数字员工是否在干活、任务进展到哪一步（独立长任务、多人协同或工作流运行）；再按需点进某个活跃员工查看正在执行的内容，或发起新任务 / 运行工作流。不是一打开就是聊天窗。

底层 AI 运行时 **只采用 AgentScope Java 2.0**，产品业务（员工、任务、工作流、知识等）都叠在这套 Java 底座上，而不是 Python AgentScope。
